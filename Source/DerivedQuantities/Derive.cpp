...
// FW WILL USE THIS FOR INTERNAL ENERGY OF THE GAS
...
// FW WILL USE THIS FOR GRAVITATIONAL POTENTIAL ENERGY OF THE GAS
    void dergaspot(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
      		    const FArrayBox& datfab, const Geometry& geomdata,
      		    Real /*time*/, const int* /*bcrec*/, int /*level*/)
          {
            ...
            // Here dat contains (density, Phi_Grav)
            amrex::ParallelFor(bx,
            [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
            {
      #ifdef GRAVITY
