...
#ifdef INFGRAV
    if (gw_interval){
      Real t_new = state[State_for_Time].curTime() + 1.26813e-5; // we must add initial physical time to get compute spectrum at the correct desired time step.                                                    
      Real t_old = state[State_for_Time].prevTime() + 1.26813e-05;
      int num_per_old = int(t_old/gw_dt);
      int num_per_new = int(t_new/gw_dt);
      const Real eps = std::numeric_limits<Real>::epsilon() * 10.0 * std::abs(t_new);
      const Real next_plot_time = (num_per_old + 1) * gw_dt;

      if ((num_per_new == num_per_old) && std::abs(t_new - next_plot_time) <= eps)
        {
          num_per_new += 1;
        }

      if ((num_per_new != num_per_old) && std::abs(t_old - next_plot_time) <= eps)
        {
          num_per_old += 1;
        }
      ...
      if (num_per_old != num_per_new)
        {
	  amrex::Print() << "COMPUTE THE SPECTRUM HERE" << std::endl;  //<< parent->levelSteps(level) << " " << t_new  << " " << num_per_old << " " << num_per_new << std::endl;                                   
          compute_grav();
        }

    }
    else {
      amrex::Print() << "Compute grav wave spectrum..." << std::endl;
      compute_grav();
    }
#endif
