...
void Nyx::advance_AxComplex_PS(amrex::Real time,
			 amrex::Real dt)
{

  BL_PROFILE("Nyx::advance_AxComplex_PS()");
	...
	for (int i = 0; i < Ax_old.nComp(); i++)                                                                                                                                                        
	  {                                                                                                                                                                                               
		...
		amrex::Abort("Ax_old has NaNs in this component::advance_AxComplex_FFT()");
	      }                                                                                                                                                                                           
	  }                                                                                                                                                                                               
      } 
	...
	for (int i = 0; i < Ax_new.nComp(); i++)                                                                                                                                                        
	  {                                                                                                                                                                                               
	    if (Ax_new.contains_nan(i,1,0))                                                                                                                                                             
	      {                                                                                                                                                                                           
		std::cout << "Testing component i for NaNs: " << i << std::endl;                                                                                                                        
		amrex::Abort("Ax_new has NaNs in this component::advance_AxComplex_FFT()");
	      }                                                                                                                                                                                           
	  }                                                                                                                                                                                               
      } 
...
#pragma omp parallel for       
  ...
  for (MFIter mfi(Ax_old,false); mfi.isValid(); ++mfi){
    ...
    amrex::ParallelFor(bx,
			 ...
			 axnew(i,j,k,Nyx::AxvRe) += std::real(temp)-lprs/2.0*(axnew(i,j,k,Nyx::AxRe)*axnew(i,j,k,Nyx::AxRe)
									      +axnew(i,j,k,Nyx::AxIm)*axnew(i,j,k,Nyx::AxIm)
									      -time*time)*axnew(i,j,k,Nyx::AxRe)*dt_c;
			 axnew(i,j,k,Nyx::AxvIm) += std::imag(temp)-lprs/2.0*(axnew(i,j,k,Nyx::AxRe)*axnew(i,j,k,Nyx::AxRe)
									      +axnew(i,j,k,Nyx::AxIm)*axnew(i,j,k,Nyx::AxIm)
									      -time*time)*axnew(i,j,k,Nyx::AxIm)*dt_c;

			 axnew(i,j,k,Nyx::AxRe)  += axnew(i,j,k,Nyx::AxvRe)*dt_d;
			 axnew(i,j,k,Nyx::AxIm)  += axnew(i,j,k,Nyx::AxvIm)*dt_d;
			 (*a)[local_indx_threaded] = complex_t(axnew(i,j,k,Nyx::AxRe),axnew(i,j,k,Nyx::AxIm));
		       });
  }
}
#endif
