...
Nyx::init_particles ()
{
...
#ifdef FDM
        else if (particle_init_type == "SphericalCollapse")
          {
            if (verbose)
              {
		amrex::Print() << "\nFW: Initializing DM particles for mixed FDM/CDM spherical collapse...\n\n";
                if (init_with_sph_particles == 1)
		  amrex::Error("FDM computations are not supported for sph particles.");
              }
            DMPC->InitSphericalCollapse(get_level(level).get_new_data(Axion_Type), level, parent->initialBaLevels()+1,Nyx::AxDens,ratio_fdm,Nyx::comoving_h,Nyx::comoving_OmM);
          }
    ...
    else if (particle_init_type == "FWMDM")
	  {
        // This new method also launches from a Binary Particle File
        amrex::Gpu::LaunchSafeGuard lsg(particle_launch_ics);
        DMPC->InitFromBinaryFile(binary_particle_file, AMREX_SPACEDIM + 1); // 
	  }
#endif
...
#ifdef FDM
amrex::Vector<double>buffer = read_binary_to_buffer("AxionRe.bin", nx * ny * nz);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        ...
        int idx = (j * (nx) * (nx) + i * (nx) + k);
          ...
          axion(i,j,k, Nyx::AxRe) = buffer[idx];
        });
    };

buffer = read_binary_to_buffer("AxionIm.bin", nx * ny * nz);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        ...
        int idx = (j * (nx) * (nx) + i * (nx) + k);
          ...
          axion(i,j,k, Nyx::AxIm) = buffer[idx];
        });
    };
    // Temp buffer.clear();
#endif 

// Now, each rank uses the data. In this case, you can still pass all ranks a subset of the data.
#ifdef _OPENMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
	for (MFIter mfi(Ax_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    // Each rank uses the global buffer data broadcasted to it
...
Nyx::particle_redistribute (int lbase, bool my_init)
{
...

    {
        ...
        if (ba.size() != flev+1)
        {
            amrex::Print() << "BoxArray Size " << ba.size() << std::endl;
            amrex::Print() << "Finest Refined Level (?)    " << flev << std::endl;
            ba.resize(flev+1);
            dm.resize(flev+1);
            changed = true;
