...
void DarkMatterParticleContainer::moveKickDrift(amrex::MultiFab &acceleration,
                                                int lev,
                                                amrex::Real dt,
                                                amrex::Real a_old,
                                                amrex::Real a_half,
...
void DarkMatterParticleContainer::moveKick(MultiFab &acceleration,
                                           int lev,
                                           Real dt,
                                           Real a_new,
                                           Real a_half)
    ...
    BoxArray &baWhereNot, int lev, int nlevs)
{
  ...
  // DEBUG
  amrex::Print() << "MultiFab has " << mf.size() << " FABs (boxes)" << std::endl;
  amrex::Print() << "MultiFab boxarray:\n";
  for (int i = 0; i < mf.boxArray().size(); ++i)
  {
      const Box& b = mf.boxArray()[i];
      amrex::Print() << "  Box " << i << ": " << b << std::endl;
  }
  ...
  Real mean_disp[AMREX_SPACEDIM] = {0};
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
int num_ranks = ParallelDescriptor::NProcs(); // Total number of ranks

  for (MFIter mfi(mf, false); mfi.isValid(); ++mfi)
  {
    int box_index = mfi.index();  // Get the current box index
    
    int rank = ParallelDescriptor::MyProc(); // Current rank index
    
    // Now process the assigned box:
    const Box &vbx = mfi.validbox();
    amrex::Print() << "Tile size: " << mfi.tilebox() << std::endl;
    //amrex::Print() << "rank " << rank << "/" << num_ranks  <<" is processing tile: " << vbx << std::endl;

    // for later if (!mfi.isOwner()) continue;
    ...
    amrex::Print()<<"LO = "<<fab_lo[2]<<" HI = "<<fab_hi[2]<<std::endl;
  ...
  amrex::AllPrint() << "Rank " << ParallelDescriptor::MyProc()
               << " created " << totalcount << " particles, redistributing." << std::endl;
  Redistribute();
}
...
void DarkMatterParticleContainer::AssignDensityAndVels(Vector<std::unique_ptr<MultiFab>> &mf, int lev_min) const
{
  AssignDensity(mf, lev_min, AMREX_SPACEDIM + 1);
}

void DarkMatterParticleContainer::InitFromBinaryMortonFile(const std::string &particle_directory,
...
#ifdef FDM
void DarkMatterParticleContainer::InitSphericalCollapse(amrex::MultiFab &mf, int lev, int nlevs, int comp, Real ratio, Real hubl, Real OmM)
{
  ...
  const Real meandens = 2.775e+11 * hubl * hubl * OmM;
    ...
    for (int kx = fab_lo[2]; kx <= fab_hi[2]; kx++)
      for (int jx = fab_lo[1]; jx <= fab_hi[1]; jx++)
        for (int ix = fab_lo[0]; ix <= fab_hi[0]; ix++)
        {
          ...
          {
            part.pos(n) = geom.ProbLo(n) + (indices[n] + Real(0.5)) * dx[n];
            part.rdata(n + 1) = 0.0;
          }
