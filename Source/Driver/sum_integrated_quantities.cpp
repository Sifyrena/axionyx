...
Nyx::compute_average_density ()
{
    ...
    for (int lev = 0; lev <= finest_level; lev++)
      {
        Nyx& nyx_lev = get_level(lev);
        // std::cout << "<<<<<<<<<<<<<<<<<<<<<<<<< LOOK HERE FOR DENSITY INFORMATION!." << std::endl;  
    ...
    if (verbose > 0 && ParallelDescriptor::IOProcessor())
    {
        if (do_hydro == 1)
        {
            std::cout << "Avg. gas density " << average_gas_density << '\n';
        }
        std::cout << "Avg. dm density " << average_dm_density << '\n';
#ifdef NEUTRINO_PARTICLES
        std::cout << "Avg. neutrino density " << average_neutr_density << '\n';
#endif
#ifdef FDM
	std::cout << "Avg. fdm density " << average_ax_density << '\n';
#endif
        std::cout << "Avg. total density " << average_total_density << '\n';
    }
