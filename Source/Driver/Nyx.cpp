...
#elif GRAVITY
int Nyx::State_for_Time = PhiGrav_Type;
#else
int Nyx::State_for_Time = State_Type;
#endif
...
Vector<Real> Nyx::fdm_halo_box;
int Nyx::mixed_cosmology = 0;
bool Nyx::shiftvel = 0;
#endif
#ifdef AXCOMPLEX
int Nyx::AxRe   = -1;
...
int Nyx::Psi = -1;
int Nyx::PsiPrime = -1;
Real Nyx::ax_init_time = -1.0;
Real Nyx::ax_time1 = 1.0;
Real Nyx::ax_n = 7.3;
Real Nyx::ax_dt = 1.0;
#endif
#ifdef INFGRAV
...
int Nyx::Sin_33 = -1;
int Nyx::NUM_S = -1;
bool Nyx::gw_interval = 0;
Real Nyx::gw_dt = 0;
Real Nyx::gw_prevtime = -1.0;
std::string Nyx::gw_log = "";
#endif
...
Nyx::read_params ()
{
    ...
    if (pp_nyx.contains("levelmethod"))
      ...
      amrex::Abort("Need to specify levelmethod for AxioNyx routines");
    ...
    pp_nyx.query("shiftvel",shiftvel);
...
#endif
#ifdef AXREAL
    pp_nyx.query("ax_time1", ax_time1);
    pp_nyx.query("ax_n", ax_n);
    pp_nyx.query("ax_dt", ax_dt);
#endif

#ifdef INFGRAV
    pp_nyx.query("gw_interval", gw_interval);
    pp_nyx.query("gw_dt", gw_dt);
    pp_nyx.get("gw_log",gw_log);
#endif
#ifdef INF
    hbaroverm = 0.00123409 / m_tt;
#endif
...
#ifdef INFGRAV
    //MultiFab&  Ax_new = get_new_data(Axion_Type);                                                                                                                                                               
    MultiFab& TRe_new = get_new_data(TensorRe_Type);
    MultiFab& TIm_new = get_new_data(TensorIm_Type);
    MultiFab& CosRe_new = get_new_data(CosRe_Type);
    MultiFab& CosIm_new = get_new_data(CosIm_Type);
    MultiFab& SinRe_new = get_new_data(SinRe_Type);
    MultiFab& SinIm_new = get_new_data(SinIm_Type);
    //FillPatch(old, Ax_new, 0, cur_time, Axion_Type, 0, NUM_AX);                                                                                                                                                 
    FillPatch(old, TRe_new, 0, cur_time, TensorRe_Type, 0, NUM_T);
    FillPatch(old, TIm_new, 0, cur_time, TensorIm_Type, 0, NUM_T);
    FillPatch(old, CosRe_new, 0, cur_time, CosRe_Type, 0, NUM_T);
    FillPatch(old, CosIm_new, 0, cur_time, CosIm_Type, 0, NUM_T);
    FillPatch(old, SinRe_new, 0, cur_time, SinRe_Type, 0, NUM_T);
    FillPatch(old, SinIm_new, 0, cur_time, SinIm_Type, 0, NUM_T);
    amrex::Print() << "FILLPATCH STATES in Nyx.cpp" << std::endl;
#endif
...
#if defined(GRAVITY) && defined(AMREX_PARTICLES)
    if (do_grav)
        particle_est_time_step(est_dt);
#endif
#ifdef AXIONYX
    axionyxEstimateTimeStep(est_dt);
#endif
    if (level==0)
...
#if !defined(AGN) && defined(AMREX_PARTICLES)
            if (do_dm_particles)
#endif
            {
...
#ifdef AMREX_PARTICLES
    if (level == lbase) {
        particle_redistribute(lbase, true);
    }
    amrex::Gpu::Device::streamSynchronize();
#endif
#ifdef GRAVITY
